...
describe WalletService::Nemd do
  ...
  describe 'WalletService::Nemd' do
    ...
    let(:sign_data) do
      Rails.root.join('spec', 'resources', 'nem-data', 'sign-transaction.json')
    ...
    let(:submit_data) do
      Rails.root.join('spec', 'resources', 'nem-data', 'submit-transaction.json')
        .yield_self {|file_path| File.open(file_path)}
        .yield_self {|file| JSON.load(file)}
        .to_json
    end
    ...
    let(:deposit_wallet) { Wallet.find_by(gateway: :nemd, kind: :deposit)}
    let(:hot_wallet) { Wallet.find_by(gateway: :nemd, kind: :hot)}

    context '#create_address!' do
      let(:service) {WalletService[wallet]}
      let(:wallet) {Wallet.find_by(gateway: :nemd, kind: :deposit)}
